...
@router.get("/",dependencies=[Depends(get_current_active_superuser)], response_model=list[User])
async def get_users(
        *,
        session:SessionDep,
        offset:int=0,
...
async def create_user(*, session:SessionDep, current_user:UserCreate):
    user_db = crud.create_user(user_create=current_user, session=session)
    return user_db
...
@router.patch("/{user_id}",dependencies=[Depends(get_current_active_superuser)], response_model=User)
...
async def update_me(*, session:SessionDep, current_user:CurrentDep, user_in:UserUpdateMe):
    if UserUpdateMe.email:
        user_new = crud.get_user_by_email(email=user_in.eamil, session=session)
        if user_new and user_new.email != user_in.eamil:
            raise HTTPException(status_code=409, detail="Email already registered")
